...
#   Author: ssrlive
#   Description: overTLS onekey for musl building and pure IP certificate
#   Version: 1.0.0
#==========================================================
...
overtls_install_sh="overtls-install-pure-ip.sh"
overtls_install_sh_url="https://raw.githubusercontent.com/shadowsocksr-live/overtls/master/install/overtls-install-pure-ip.sh"
    ...
    cat > ${local_cfg_file_path} <<EOF
{
    ...
    "client_settings": {
        ...
        "server_domain": "${web_svr_local_ip_addr}",
...
function request_pure_ip_cert() {
    ...
    # 证书的各种文件存储在 ~/.acme.sh/${hostaddr}_ecc 目录下， 其中 ${hostaddr} 是你当前主机的公网 IP 地址。
    local hostaddr=${web_svr_local_ip_addr}
    letsencrypt_cert_file="${HOME}/.acme.sh/${hostaddr}_ecc/fullchain.cer"
    letsencrypt_key_file="${HOME}/.acme.sh/${hostaddr}_ecc/${hostaddr}.key"
}
...
function cron_random_restart_overtls_svc() {
    ...
    local restart_job="${random_minute} ${random_hour} * * * systemctl restart overtls"

    if crontab -l 2>/dev/null | grep -Fq "systemctl restart overtls"; then
        echo -e "${OK} ${GreenBG} Overtls restart cron job already exists, skipping add. ${Font}"
        return 0
    fi

    (crontab -l 2>/dev/null; echo "${restart_job}") | crontab -
}

function input_web_listen_port() {
    local port=$1
    stty erase '^H' && read -p "Please enter the access port number (default: ${port}):" port
    [[ -z ${port} ]] && port=$1
    echo ${port}
}
...
function install_overtls_remote_server() {
    ...
    svr_listen_port=`input_web_listen_port ${svr_listen_port}`

    local rvs_path=${reverse_proxy_location}
    echo "请输入 代理入口路径(不带前后斜杠), 默认值 ${rvs_path} "
...
function main() {
    echo
    echo "####################################################################"
    echo "# Script of Install ${service_name} Server with pure IP certificate"
    echo "# Author: ssrlive"
